...
## DR Plan: Restore Project (data)

Полный план восстановления всех приложений в ArgoCD project.

### Namespaces в project "data":
- example-api-dev
- example-api-prd
- example-ui-dev
- example-ui-prd
...
# 2. Выбери нужный backup
velero backup describe <backup-name> --details

# 3. Удали ArgoCD Applications (останавливает GitOps sync)
kubectl delete application \
  example-api-dev \
  example-api-prd \
  example-ui-dev \
  example-ui-prd \
  example-api-main-db-dev \
  example-api-main-db-prd \
  example-api-cache-dev \
  example-api-cache-prd \
  -n argocd
...
# 6. Restore
velero restore create project-data-restore \
  --from-backup <backup-name> \
  --include-namespaces example-api-dev,example-api-prd,example-ui-dev,example-ui-prd \
  --wait

# 7. Проверь restore
velero restore describe project-data-restore
kubectl get pods -n example-api-dev
kubectl get pods -n example-api-prd
kubectl get pvc -A | grep example

# 8. ArgoCD root app автоматически пересоздаст Applications
kubectl get applications -n argocd | grep example

# 9. Проверь данные в БД
